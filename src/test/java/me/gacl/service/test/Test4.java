...
public class Test4 {
    ...
    public void testGetClass3() {
        ...
        /**
         * 映射sql的标识字符串，
         * me.gacl.dao.ClassMapper是classMapper.xml文件中mapper标签的namespace属性的值，
         * getClass3是select标签的id属性值，通过select标签的id属性值就可以找到要执行的SQL
         */
        String statement = "me.gacl.dao.ClassMapper.getClass3";//映射sql的标识字符串
    ...
    public void testGetClass4() {
         ...
         */
        String statement = "me.gacl.dao.ClassMapper.getClass4";//映射sql的标识字符串
        //执行查询操作，将查询结果自动封装成Classes对象返回
        Classes clazz = sqlSession.selectOne(statement, 1);//查询class表中id为1的记录
        //使用SqlSession执行完SQL之后需要关闭SqlSession
        sqlSession.close();
